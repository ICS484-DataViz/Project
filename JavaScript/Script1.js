...
dfd.readCSV('../DataStuff/steam.csv').then((df) => {
  ...
  for (let i = 0; i < genre.length; i++) {
    const input = genre[i].split(';');
    const ownerTemp = owners[i].split('-');
    for (let x = 0; x < input.length; x++) {
      if (input[x] != 'Indie') {
        genreOwners.push({'genre': input[x], 'owners': ownerTemp[0]});
      }
    }
  }
...
*/
  ...
  */
  ...
  uniqGenres.forEach(doWorkGenreOwners); // totals up each genres
  console.log(genreOwnersT);
  genreOwnersT.sort((a, b) => // sorts the genres by size
  ...
  for (let a = 0; a < 11; a++) {
    genreOwnerX.push(Object.keys(genreOwnersT[a]).toString());
    genreOwnerY.push(Object.values(genreOwnersT[a])[0]);
  }

  const genreOwnerTrace = {
    x: genreOwnerX,
    y: genreOwnerY,
    type: 'bar',
    name: 'Most Popular Genre by Owners'
  }
 ...
 */
  ...
  const genreOwnerData = [genreOwnerTrace]
...
const doWorkGenreOwners = (value) => {
  const temp = genreOwners.filter(item => item.genre === value);
  const name = value;
  const data ={};
  data[name] = temp.reduce((total, current) => Number(total) + Number(current.owners), 0);
  genreOwnersT.push(data);
}
